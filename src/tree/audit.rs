...
impl Tree {
    pub async fn audit(&self, start: u64, limit: u64) -> Result<(Vec<AuditorUpdate>, bool)> {
        ...
        if start > current_size {
            return Err(anyhow!("Auditing cannot start past end of tree"));
        }
        if start == current_size {
            return Ok((vec![], false));
        }
    ...
    pub async fn set_auditor_head(
        &mut self,
        head: PbAuditorTreeHead,
        ...
        if head.tree_size < current_size {
            consistency = self
                .log
                .get_consistency_proof(head.tree_size, current_size)?;
        }

        self.auditors.insert(
            matched_auditor_name,
            AuditorTreeHead {
