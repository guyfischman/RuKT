...
use crate::proto::transparency::{
    Consistency, ContactMonitorRequest, ContactMonitorResponse, DistinguishedRequest,
    DistinguishedResponse, MonitorMapEntry, OwnerInitRequest, OwnerInitResponse,
...
impl Tree {
        ...
        for entry in entries {
            if entry.position >= tree_size {
                return Err(anyhow::Error::new(KtError::InvalidArgument(format!(
                    "Entry position {} outside tree of size {}",
    ...
    pub async fn contact_monitor(
        &self,
        req: &ContactMonitorRequest,
    ) -> Result<ContactMonitorResponse> {
        let tree_size = self.monitored_tree_size()?;
        Self::validate_monitor_entries(&req.entries, tree_size)?;

        let proof = self
            .traverse_contact_monitoring(tree_size, &req.label, &req.entries, req.last.unwrap_or(0))
            .await?;
    ...
    pub async fn owner_init(&self, req: &OwnerInitRequest) -> Result<OwnerInitResponse> {
        ...
        if req.start >= tree_size {
            return Err(anyhow::Error::new(KtError::InvalidArgument(format!(
                "Start position {} outside tree of size {}",
                req.start, tree_size
            ))));
        }
        // TODO: verify start is unexpired and distinguished (§13.3)

        let (proof, binary_ladder, greatest_versions) = self
            .traverse_owner_init(tree_size, &req.label, req.start, req.last.unwrap_or(0))
            .await?;
        let fth = self.get_full_tree_head(Some(Consistency {
            last: req.last,
            distinguished: None,
    ...
    pub async fn distinguished(&self, req: &DistinguishedRequest) -> Result<DistinguishedResponse> {
        ...
        let proof = self
            .traverse_distinguished(tree_size, req.stop, req.last.unwrap_or(0))
            .await?;
        let fth = self.get_full_tree_head(Some(Consistency {
            last: req.last,
            distinguished: None,
    ...
    pub async fn owner_monitor(&self, req: &OwnerMonitorRequest) -> Result<OwnerMonitorResponse> {
        ...
        let (proof, binary_ladder) = self
            .traverse_owner_monitor(
                tree_size,
                &req.label,
                &req.entries,
                req.start,
