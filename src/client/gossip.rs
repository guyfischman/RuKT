use crate::crypto::{
    PublicConfig, ServiceVerifyingKey, construct_tree_head_tbs_public, verify_data,
};
...
impl GossipHead {
    pub fn new(tree_size: u64, root_hash: &[u8], head: &TreeHead) -> Self {
        let mut buf = Vec::new();
        head.encode(&mut buf)
            .expect("prost encoding is infallible for TreeHead");
        Self {
            tree_size,
...
pub fn verify_fork_evidence(config: &PublicConfig, evidence: &ForkEvidence) -> Result<()> {
    ...
    for (root, head) in [
        (&evidence.root_a, &evidence.head_a),
        (&evidence.root_b, &evidence.head_b),
    ] {
