...
async fn test_update_response_structure_compliance() -> Result<()> {
    ...
    assert_eq!(update_v0.timestamps.len(), 1, "Genesis update should have 1 timestamp (self)");
    assert_ne!(update_v0.timestamps[0], 0, "Timestamp should be real (non-zero epoch for genesis if possible, though tests run fast)");
    ...
    assert_eq!(update_v1.timestamps.len(), 2, "Should contain timestamps for previous frontier (0) and new node (1)");
    ...
    assert!(!update_v1.prefix_proofs.is_empty(), "Must contain prefix proofs for verification");
