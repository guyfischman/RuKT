...
async fn test_randomized_vrf_proofs() -> Result<()> {
    ...
    assert_eq!(output_1, output_2, "VRF Output (Index) must be deterministic");
    assert_ne!(proof_1, proof_2, "VRF Proof bytes must be randomized to prevent traffic correlation");
