...
use crate::proto::transparency::{UpdateRequest, TreeSearchRequest, Consistency, FullTreeHeadType, SignedUpdateRequest};
...
async fn test_full_tree_head_same_optimization() -> Result<()> {
    ...
    let fth_same = resp_same.tree_head.unwrap();
    ...
    let req_behind = tonic::Request::new(TreeSearchRequest {
        search_key: user.clone(),
        consistency: Some(Consistency {
            last: Some(1),
            distinguished: None
    ...
    let fth_behind = resp_behind.tree_head.unwrap();
