...
use crate::proto::transparency::{UpdateRequest, LabelValue, DistinguishedRequest};
...
async fn test_walk_distinguished_heads() -> Result<()> {
    ...
    for i in 0..7 {
        service.update(tonic::Request::new(UpdateRequest {
            ...
            values: vec![LabelValue { value: b"v".to_vec() }],
        })).await?;
    }

    let resp = service.distinguished(tonic::Request::new(DistinguishedRequest {
        last: None,
        stop: None,
    })).await?.into_inner();
    ...
    assert!(stopped_proof.timestamps.len() < 7, "Stopping position must truncate the walk");
