...
async fn test_client_rejects_tampered_search_responses() -> Result<()> {
    ...
    server.client().await?.verify_search_response(&label, None, &good).await
        .expect("honest response must verify");

    let mutations: Vec<(&str, Box<dyn Fn(&mut SearchResponse)>)> = vec![
        ("value bytes", Box::new(|r: &mut SearchResponse| {
            r.value.as_mut().unwrap().value = b"forged".to_vec();
        })),
        ("commitment opening", Box::new(|r: &mut SearchResponse| {
            r.opening[0] ^= 0xff;
        })),
        ("reported version", Box::new(|r: &mut SearchResponse| {
            r.version = Some(r.version.unwrap() + 1);
        })),
        ("ladder vrf proof", Box::new(|r: &mut SearchResponse| {
            r.binary_ladder[0].proof[0] ^= 0xff;
        })),
        ("ladder commitment", Box::new(|r: &mut SearchResponse| {
            for step in r.binary_ladder.iter_mut() {
                if let Some(c) = step.commitment.as_mut() { c[0] ^= 0xff; break; }
            }
        })),
        ("prefix proof element", Box::new(|r: &mut SearchResponse| {
            for p in r.search.as_mut().unwrap().prefix_proofs.iter_mut() {
                if let Some(e) = p.elements.get_mut(0) { e[0] ^= 0xff; return; }
            }
        })),
        ("log timestamp", Box::new(|r: &mut SearchResponse| {
            if let Some(ts) = r.search.as_mut().unwrap().timestamps.last_mut() { *ts += 1; }
        })),
        ("inclusion proof", Box::new(|r: &mut SearchResponse| {
            let inc = r.search.as_mut().unwrap().inclusion.as_mut().unwrap();
            if let Some(e) = inc.elements.get_mut(0) { e[0] ^= 0xff; }
        })),
        ("head signature", Box::new(|r: &mut SearchResponse| {
            let th = r.full_tree_head.as_mut().unwrap().tree_head.as_mut().unwrap();
    ...
    for (name, mutate) in mutations {
        ...
        let result = server.client().await?
            .verify_search_response(&label, None, &tampered).await;
        assert!(result.is_err(), "client must reject tampered field: {}", name);
    }
