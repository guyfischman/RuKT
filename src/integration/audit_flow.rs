...
async fn test_audit_flow_with_signatures() -> Result<()> {
    ...
    let service =
        KeyTransparencyImpl::new(db, server_signer, vrf_key, auditor_map.clone(), None).await?;
    ...
    // 2. Audit the log
    let resp = service
        .audit(tonic::Request::new(AuditRequest {
            start: 0,
            limit: 10,
        }))
        .await?;
    ...
    service
        .set_auditor_head(tonic::Request::new(auditor_head))
        .await?;
...
async fn test_auditor_verifies_multi_leaf_batches() -> Result<()> {
    ...
    let service =
        KeyTransparencyImpl::new(db, server_signer, vrf_key, HashMap::new(), None).await?;
    ...
    assert!(
        updates[0]
            .added
            .windows(2)
            .all(|w| w[0].vrf_output < w[1].vrf_output)
    );
    ...
    assert_eq!(
        prefix_root, served_root,
        "Auditor-computed prefix root must match the operator's"
    );
