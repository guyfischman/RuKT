...
use crate::proto::transparency::{AuditorTreeHead, LabelValue, UpdateRequest};
...
async fn test_malicious_auditor_logic() -> Result<()> {
    ...
    let auditor_raw_signer = if let crypto::ServiceSigningKey::Ed25519(s) = auditor_signer {
        s
    } else {
        panic!()
    };
    let auditor_raw_vk = if let crypto::ServiceVerifyingKey::Ed25519(v) = auditor_vk {
        v
    } else {
        panic!()
    };
    ...
    auditor_map.insert(
        auditor_pk_bytes.clone(),
        crypto::ServiceVerifyingKey::Ed25519(auditor_raw_vk),
    );
    ...
    assert!(
        res_future.is_err(),
        "Server must reject TreeHead size > local tree size"
    );
    assert!(
        res_future
            .unwrap_err()
            .message()
            .contains("ahead of service")
    );
    ...
    assert!(
        res_rewind.is_err(),
        "Server must reject auditor rewind attempt"
    );
    let err_msg = res_rewind.unwrap_err().message().to_string();
    assert!(
        err_msg.contains("regression"),
        "Error message should mention regression/rewind. Got: {}",
        err_msg
    );

    // Verify the state remains at 2
    let tree_guard = service.tree.write().await;
    let stored_ath = tree_guard
        .auditors
        .get(&hex::encode(&auditor_pk_bytes))
        .unwrap();

    assert_eq!(
        stored_ath.tree_size, 2,
        "Auditor state should verify remain at 2"
    );
