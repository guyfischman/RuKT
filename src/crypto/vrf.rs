...
fn ecvrf_verify_ed25519(pub_key: &[u8], alpha: &[u8], proof: &[u8]) -> Result<[u8; 32]> {
    if proof.len() != 80 {
        return Err(anyhow!("Invalid Ed25519 VRF proof length"));
    }
    ...
    let gamma = CompressedEdwardsY::from_slice(gamma_bytes)
        .map_err(|_| anyhow!("Invalid Gamma"))?
        .decompress()
...
fn expand_vrf_secret_p256(secret: &[u8]) -> Result<VrfContext> {
    if secret.len() != 32 {
        return Err(anyhow!("Invalid P-256 scalar length"));
    }
    let mut arr = [0u8; 32];
...
fn ecvrf_verify_p256(pub_key: &[u8], alpha: &[u8], proof: &[u8]) -> Result<[u8; 32]> {
    if proof.len() != 81 {
        return Err(anyhow!("Invalid P-256 VRF proof length"));
    }
    let gamma_bytes = &proof[0..33];
    let c_bytes = &proof[33..49];
    let s_bytes = &proof[49..81];

    let gamma = Option::<ProjectivePoint>::from(ProjectivePoint::from_encoded_point(
        &p256::EncodedPoint::from_bytes(gamma_bytes).map_err(|_| anyhow!("Invalid Gamma"))?,
    ))
    ...
    s_arr.copy_from_slice(s_bytes);
    let s: P256Scalar =
        Option::from(P256Scalar::from_repr(s_arr.into())).ok_or_else(|| anyhow!("Invalid s"))?;

    let pk_point = Option::<ProjectivePoint>::from(ProjectivePoint::from_encoded_point(
        &p256::EncodedPoint::from_bytes(pub_key).map_err(|_| anyhow!("Invalid PK"))?,
    ))
